...
const topicFormSchema = z.object({
  ...
  default_language: z.string().min(1, { message: "Default language is required." }),
});

// --- Step 2: Group, Contact, Email Template Schema ---
...
export const AddTopicDialog: React.FC = () => {
  ...
  const handleTopicSubmit = async (values: z.infer<typeof topicFormSchema>) => {
    try {
      ...
      showSuccess('Topic created successfully! Now add a contact.');
      setStep(2);
    } catch (error) {
  ...
  const handleContactSubmit = async (values: z.infer<typeof contactFormSchema>) => {
    ...
    try {
      ...
      const contactData: Omit<Contact, 'id'> = {
        ...
        organization: values.contactOrganization || null,
        location: values.contactLocation || null,
        emoji: values.contactEmoji || '👤', // Default emoji if none provided
        email: values.contactEmail,
        languages: values.contactLanguages,
      };
      ...
      showSuccess('Group, contact, and email template added successfully!');
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-[500px] rounded-xl p-6 bg-card text-card-foreground">
        <DialogHeader>
          ...
          <DialogDescription className="text-muted-foreground">
            {step === 1
              ...
              : 'Now, add a group, a key contact, and an email template for this topic.'}
          </DialogDescription>
        </DialogHeader>
          ...
          <Form {...topicForm}>
            <form onSubmit={topicForm.handleSubmit(handleTopicSubmit)} className="grid gap-4 py-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-sm font-medium text-foreground">Default Language</FormLabel>
              ...
              <DialogFooter className="pt-4">
                <Button type="submit" className="w-full rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground text-base py-3">
                  Next: Add Contact <CheckCircle2 className="ml-2 h-4 w-4" />
                </Button>
              </DialogFooter>
            </form>
          </Form>
        ...
        {step === 2 && (
          <Form {...contactForm}>
            <form onSubmit={contactForm.handleSubmit(handleContactSubmit)} className="grid gap-4 py-4">
              {/* Group Details */}
              <h3 className="text-lg font-semibold text-foreground mt-2">Group Details</h3>
              <FormField
