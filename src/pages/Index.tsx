...
const TopicCard: React.FC<{ topic: Topic }> = ({ topic }) => { // Use Topic type
  return (
    <Link to={`/${topic.slug}`} className="block h-full">
      <Card className="h-full flex flex-col justify-between rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out border-2 border-transparent hover:border-primary bg-card text-card-foreground">
      ...
      <div className="container mx-auto max-w-6xl py-12">
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[...Array(6)].map((_, i) => (
              <Card key={i} className="rounded-xl shadow-lg border-2 border-border bg-card">
                <CardHeader>
