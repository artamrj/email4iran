...
const ContactCard: React.FC<{ contact: Contact; personalization: Personalization }> = ({ contact, personalization }) => {
  ...
  return (
    <Card className="rounded-xl shadow-md border-none bg-gradient-to-br from-iran-white/80 to-iran-white dark:from-background dark:to-background p-4 flex flex-col justify-between h-full">
      ...
      <CardFooter className="p-0 pt-4 flex flex-col sm:flex-row gap-2">
        ...
        <Dialog>
          <DialogTrigger asChild>
            <Button variant="outline" className="w-full sm:w-auto flex-grow rounded-lg border-primary text-primary hover:bg-primary/10 dark:border-primary/70 dark:text-primary/70 dark:hover:bg-background/20 text-sm py-2 px-3">
          ...
          <DialogContent className="sm:max-w-[425px] rounded-xl p-6">
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  className="rounded-lg border-border dark:border-border bg-input dark:bg-input text-foreground"
                />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="body" className="text-sm font-medium text-foreground/80">Body</Label>
                <Textarea
                  id="body"
                  value={body}
                  onChange={(e) => { /* In a real app, you might manage this state */ }}
            ...
            <DialogFooter className="flex flex-col sm:flex-row gap-2">
              ...
              <Button
                ...
                className="w-full sm:w-auto rounded-lg border-accent text-accent hover:bg-accent/10 dark:border-accent/70 dark:text-accent/70 dark:hover:bg-background/20 text-sm py-2 px-3"
                disabled={!subject || !body}
              >
...
const TopicDetail = () => {
  ...
  if (isLoadingTopic || isLoadingGroups || isLoadingContacts) {
    return (
      <div className="min-h-screen flex flex-col bg-gradient-to-br from-iran-white to-iran-white/90 dark:from-background dark:to-background p-4 sm:p-8">
        <div className="container mx-auto max-w-6xl py-8">
          <Button variant="ghost" onClick={() => navigate(-1)} className="mb-6 rounded-full text-foreground/70 hover:bg-muted dark:hover:bg-muted/20">
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-iran-white to-iran-white/90 dark:from-background dark:to-background p-4 sm:p-8">
      <div className="container mx-auto max-w-6xl py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <Button variant="ghost" onClick={() => navigate(-1)} className="rounded-full text-foreground/70 hover:bg-muted dark:hover:bg-muted/20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Card className="lg:col-span-2 rounded-xl shadow-lg border-none bg-gradient-to-br from-iran-white/80 to-iran-white dark:from-background dark:to-background p-6">
            <CardHeader className="p-0 pb-4">
              <CardTitle className="text-4xl sm:text-5xl font-extrabold text-iran-green dark:text-iran-green-foreground leading-tight mb-2">
                {topic.name}
              </CardTitle>
            </CardHeader>
          ...
          <Card className="lg:col-span-1 rounded-xl shadow-lg border-none bg-gradient-to-br from-iran-white/80 to-iran-white dark:from-background dark:to-background p-6">
            <CardHeader className="p-0 pb-4">
            ...
            <CardContent className="p-0 grid gap-4">
              ...
              <div>
                <Label htmlFor="city" className="text-sm font-medium text-foreground/80">Your City (Optional)</Label>
                <Input
                  id="city"
              ...
              <div>
                <Label htmlFor="country" className="text-sm font-medium text-foreground/80">Your Country (Optional)</Label>
                <Input
                  id="country"
              ...
              <div className="flex items-center justify-between space-x-2">
                <Label htmlFor="always-english" className="text-sm font-medium text-foreground/80">Always use English</Label>
                <Switch
                  id="always-english"
                  ...
                  className="data-[state=checked]:bg-primary data-[state=unchecked]:bg-secondary dark:data-[state=unchecked]:bg-muted"
                />
              </div>
              <Button
                onClick={handleCopyAllEmails}
                className="w-full rounded-lg bg-accent hover:bg-accent/90 text-accent-foreground text-base py-3 mt-4"
              >
        ...
        {/* Groups & Contacts */}
        <div className="mt-12">
          <h2 className="text-3xl font-bold text-iran-green dark:text-iran-green-foreground mb-6">Key Contacts</h2>
          {groups?.map((group) => (
            <div key={group.id} className="mb-10">
              <h3 className="text-2xl font-semibold text-foreground mb-4">
                {group.name}
              </h3>
              <p className="text-foreground/80 mb-6">{group.description}</p>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {contactsByGroup?.[group.id]?.map((contact) => (
                  <ContactCard key={contact.id} contact={contact} personalization={personalization} />
                ))}
              </div>
              <Separator className="my-8 bg-border dark:bg-border rounded-full" />
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Footer */}
      <footer className="w-full py-6 text-center text-foreground/70 text-sm mt-auto">
